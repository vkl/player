...
processData(struct MessageQueueItem **msgQueueItem,
    ...
    printf("%s %.100s -> %s\n\r", cast_msg->source_id, cast_msg->payload_utf8, cast_msg->destination_id);
...
enum CastMessageType 
sendMessage(struct MessageQueueItem **msgQueueItem, struct MessageItem **msgItem, struct CastState *cs) {
    
    enum CastMessageType retval = NONE;

    if ((*msgQueueItem) == NULL) return retval;
    ...
    printf("%s -> %s %.100s\n\r", cast_msg.source_id, cast_msg.destination_id, cast_msg.payload_utf8);
    size_t size = api__cast_message__get_packed_size(&cast_msg);
   
    (*msgItem) = (struct MessageItem*)calloc(1, sizeof(struct MessageItem));
    if ((*msgItem) == NULL) {
        printf("calloc error for MessageItem\n\r");
        goto end;
    }
